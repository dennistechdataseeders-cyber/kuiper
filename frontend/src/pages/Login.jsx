...
const ParticleSwarm = () => {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // 1. Define the base "Flat" Circle in the XZ plane
        let x0 = Math.cos(angle) * baseRadius;
        let z0 = Math.sin(angle) * baseRadius;
        let y0 = (Math.random() - 0.5) * 12; // Internal thickness of the disk

        // 2. Apply Diagonal Tilt
        // tiltZ is now POSITIVE to pull the left side down and push the right side up
        const tiltX = Math.PI / 15;   
        const tiltZ = Math.PI / 7;  

        // Rotation Math (Z-axis rotation first, then X-axis rotation)
        ...
        // Color: Shift based on position for a "nebula" effect
...
const BackgroundScene = () => (
  <div className="absolute inset-0 z-0 overflow-hidden">
    <Canvas camera={{ position: [0, 0, 150], fov: 60 }} dpr={[1, 2]}>
      ...
      <ParticleSwarm />
      {/* OrbitControls are still present but rotation/zoom/pan are disabled */}
...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#020617] p-4 overflow-hidden relative">
      ...
      <form 
        ...
        className="relative z-10 bg-white/5 backdrop-blur-3xl p-8 md:p-12 rounded-[3rem] shadow-2xl w-full max-w-md border border-white/10"
      >
  ...
  <div className="relative w-24 h-24 mx-auto mb-6">
    ...
    {/* Main Container */}
    <div className="relative w-full h-full bg-blue-600/20 
      border border-blue-500/30 rounded-3xl 
      flex items-center justify-center 
      shadow-2xl shadow-blue-500/10 backdrop-blur-md overflow-hidden">
  ...
  {/* KUIPER */}
  <span className="font-[Orbitron] text-3xl tracking-tight 
    bg-gradient-to-t to-white via-blue-400 from-cyan-400 
  ...
  <p className="text-slate-400 font-bold text-xs tracking-[0.3em] mt-2">
    Engineered for Operations
        ...
        <div className="space-y-6">
          ...
          <p className="justify-center mt-3 text-[11px] font-semibold tracking-[0.25em] uppercase 
