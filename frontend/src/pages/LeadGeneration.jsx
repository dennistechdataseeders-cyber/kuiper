...
const LeadCard = ({ lead, onQuickAction }) => {
      ...
      >
        <div className="flex items-center gap-5">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-3 mb-1.5">
              ...
              <span
                className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg border ${getStatusStyles(
                  lead.status
                )}`}
              >
                {lead.status || 'New'}
              ...
              <span className="text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg bg-blue-600 text-white">
                {lead.leadType}
              </span>
            </div>
        ...
        <div className="flex items-center gap-6">
          <div className="hidden sm:block text-right">
            <p className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-1 justify-end mb-1">
              Created On
            </p>
          ...
          <div>
            ...
            <div className="mt-6 flex flex-col gap-3">
              <button
                ...
                className="w-full bg-orange-500 hover:bg-orange-600 text-white text-xs font-black uppercase tracking-widest py-3 rounded-2xl transition-all shadow-lg shadow-orange-100 active:scale-95"
              >
                Take Action Now

                <div className="text-[9px] font-bold opacity-80 mt-1">
                  Set Follow-up For Today
                </div>
              </button>
              ...
              <button
                ...
                className="w-full bg-purple-600 hover:bg-purple-700 text-white text-xs font-black uppercase tracking-widest py-3 rounded-2xl transition-all shadow-lg shadow-purple-100 active:scale-95"
              >
                Send To Feasibility

                <div className="text-[9px] font-bold opacity-80 mt-1">
                  Set Feasibility Date To Today
                </div>
              </button>
            </div>
          </div>
...
const LeadGeneration = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [orgRes, leadRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/api/orgs`, {
          headers
        }),

        axios.get(
          `${API_BASE_URL}/api/lead-generation`,
          { headers }
        )
      ]);
      ...
      setGeneratedLeads(
        Array.isArray(rawLeads) ? rawLeads : []
      );
  ...
  useEffect(() => {
    if (prospectData) {
      ...
      const orgId =
        prospectData.organizationId?._id ||
  ...
  const handleQuickAction = async (
    ...
    try {
      ...
      if (actionType === 'Feasibility') {
        payload = {
          status: 'Feasibility',
          feasibilityDate: today
        };
      }

      await axios.patch(
        `${API_BASE_URL}/api/lead-generation/${leadId}/action`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const prospectId =
        prospectData?._id ||
        window.history.state?.usr?.prospect
          ?._id;
