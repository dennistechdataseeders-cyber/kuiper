...
import { 
  ...
  Activity, Filter, MessageSquare, Eye, Send,
  Loader2, Clock, User, Paperclip, Image, File,
  Download, Trash2
} from 'lucide-react';
...
const DeveloperProjects = () => {
  ...
  const canDeleteComment = (comment) => {
    ...
    return userRole === 'Admin' || userRole === 'Project Manager' || commentUserId === userId;
  };
                ...
                {loadingComments ? (
                  ...
                  projectComments.map((comment) => {
                    ...
                    return (
                      <div
                        key={comment._id}
                        className={`p-4 rounded-xl border ${isOwn ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-200'} transition-all`}
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1.5">
                              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white font-bold text-xs ${isOwn ? 'bg-blue-600' : 'bg-slate-400'}`}>
