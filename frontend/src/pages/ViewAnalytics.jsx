...
const ViewAnalytics = () => {
  ...
  const getBadgeColor = (type) => {
    if (type?.includes('CREATED')) return 'bg-emerald-50 text-emerald-600 border-emerald-100';
    if (type?.includes('DELETE')) return 'bg-rose-50 text-rose-600 border-rose-100';
    if (type?.includes('UPDATE')) return 'bg-amber-50 text-amber-600 border-amber-100';
    return 'bg-blue-50 text-blue-600 border-blue-100';
  };

  return (
    <div className="lg:ml-64 p-6 bg-slate-50 min-h-screen font-sans">
        ...
        <div className="space-y-2">
          {loading && logs.length === 0 ? (
            ...
            logs.map((log) => (
              <div 
                key={log._id} 
                className={`group bg-white border transition-all duration-200 overflow-hidden ${
                  expandedLog === log._id 
              ...
              >
                ...
                >
                  <div className="flex items-center gap-4 flex-1">
                    <div className={`text-[9px] font-black px-2 py-1 rounded-md border uppercase tracking-tight ${getBadgeColor(log.actionType)}`}>
                      {log.actionType?.replace('_', ' ')}
                    </div>
                    <div className="flex items-center gap-2 text-slate-400">
                      <Clock size={12} />
                      <span className="text-[11px] font-bold">{new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <h3 className={`text-sm font-bold truncate transition-colors ${expandedLog === log._id ? 'text-blue-600' : 'text-slate-600'}`}>
                      {log.details}
                    </h3>
                  </div>
